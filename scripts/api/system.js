  ...
  render () {
    ...
    return (<div>
      ...
      <Action label='Get Properties' action={system.getProperties}>
        <div>
          <select multiple={true} key='arg0' defaultValue={this.props.kodi.types.system.property}>
            <option disabled>Properties</option>
            {this.props.kodi.types.system.property.sort().map(p=> <option key={p} value={p}>{p}</option>)}
          </select>
        </div>
      </Action>
